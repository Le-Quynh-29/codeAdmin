...
(function ($, window, document, Math, undefined) {
  ...
  var chartinator = function chartinator(el, options) {
    //  The chartinator object
    var o = this; //  Define table and chart elements	

    var $tableS = $(el);
    var $chartS = $tableS; //  Define fonts

    o.fontFamily = $('body').css('font-family').replace(/["']{1}/gi, "") || 'Arial, Helvetica, sans-serif'; //  Initialize option defaults ------------------------------------------------------------
    ...
    o.chartOptions = {}; // The Google Chart options object clone

    o.cchartOptions = {}; // Window resize event timer function

    o.timer = false; // Chart Id - The id to apply to the chart container element

    o.chartId = o.optionsInit.chartId; // Table Id - The id to apply to the table element

    o.tableId = o.optionsInit.tableId; // The table has data boolean

    o.tableHasData = false;
    o.tableCaption = false; // The Google Sheet data object - Data returned

    o.googleSheetData = false; // Data array - the array of collected data to send to Google Charts

    o.dataArray = []; // Set chartPackage - Default: corechart - The Google Chart Package to load.

    o.chartPackage = 'corechart'; // Array of chart types included in the Google Charts corechart package

    o.coreCharts = ['BarChart', 'ColumnChart', 'PieChart', 'AreaChart', 'BubbleChart', 'CandlestickChart', 'ComboChart', 'Histogram', 'LineChart', 'ScatterChart', 'SteppedAreaChart']; // Init chart parent

    o.chartParent = false; // Init the window width

    o.windowWidth = false; // Init chart parent width

    o.chartParentWidth = false; //  Initiate Chart ======================================================================

    o.init = function (el, options) {
      //  Merge options
      o.options = $.extend(true, {}, o.optionsInit, options); // Update chartId

      o.chartId = options.chartId || $chartS.attr('id') || o.options.chartId; // Define table and chart elements --------------------------------------------------
      // Set table element
      ...
      if ($chartS[0] === $tableS[0]) {
        // table and chart are the same element
        if (o.tableHasData) {
          // chart element does not exist
          // Reset Chart id
          o.chartId = o.options.chartId; // Insert a new chart element after the table
      ...
      $chartS.addClass(o.chartId + ' ' + o.options.chartClass).attr('id', o.chartId); // Add table class and id and get caption

      if (o.tableHasData) {
        // Update tableId
        o.tableId = options.tableId || $tableS.attr('id') || o.options.tableId; // Apply id and classes to table

        $tableS.addClass(o.tableId + ' ' + o.options.tableClass).attr('id', o.tableId);
      ...
      o.chartParent = $chartS.parent(); // Get data ----------------------------------------------------------
    ...
    o.setupChart = function () {
      ...
      if (o.options.chartOptions) {
        // Use chartOptions object if it exists
        o.chartOptions = o.options.chartOptions;
      } else if (o.options[o.camelCase(o.options.chartType)]) {
        // Use options specific to the chart type if they exist
        o.chartOptions = o.options[o.camelCase(o.options.chartType)];
      } // Clone Google Chart options so we don't overwrite original values


      o.cchartOptions = $.extend(true, {}, o.chartOptions); // Create table -------------------------------------------------------
    ...
    o.collectData = function () {
      var dataArray = []; // Format Google Sheet data
      ...
      if (o.options.columns) {
        if (dataArray[0] && dataArray[0][0] && dataArray[0][0].label) {
          // header data exists
      ...
      if (o.options.rows && dataArray.length) {
        // js data array exists
        if (o.options.colIndexes) {
          // colIndexes array exists
          for (var i = 0; i < o.options.rows.length; i++) {
            // loop through each row in js data array
            for (var j = 0; j < o.options.colIndexes.length; j++) {
          ...
          // colIndexes array does not exist
    ...
    o.drawChart = function () {
      ...
      if (!o.data || !o.data.getNumberOfRows()) {
        // No data
        // Show table remove chart
        o.showTableChart('show', 'remove');
      ...
      o.setDimensions(); // Revise Chart Options -------------------------------------------------------------

      if (o.options.chartType === 'BarChart' && !o.options.chartAspectRatio && !o.chartOptions.height) {
        // Height not set
        var fontSize = o.cchartOptions.fontSize || o.getFontSize('body', 16); // Define height based on font size and number of rows
      ...
      o.chart = new google.visualization[o.options.chartType]($chartS.get(0)); // Add ready and error event listeners

      google.visualization.events.addListener(o.chart, 'ready', function (e) {
        // Show chart
        o.showTableChart(o.options.showTable, 'show'); // Store the window width

        o.windowWidth = $(window).width(); // Store the chart parent width

        o.chartParentWidth = o.chartParent.width(); // Zoom and offset chart

        if (o.options.chartZoom || o.options.chartOffset) {
          // The CSS3 transform value
          var transform = ''; // The chart canvas object

          var $chartCanvas = $chartS.children(':last'); // The top and left css values to be applied to the tooltip

          var top = 0;
          var left = 0; // The zoom and offset values
          ...
          offsetY = offsetY * $chartS.height() * zoom / 100; // Zoom
          ...
          $chartS.css('overflow', 'hidden'); // Add style to head to Adjust tooltip

          $('<style> .' + o.chartId + ' .google-visualization-tooltip { ' + 'top: ' + top + 'px !important; ' + 'left: ' + left + 'px !important; ' + 'transform: scale(' + tooltipZoom + ')} </style>').appendTo('head');
        ...
        if (o.options.chartType === 'Calendar') {
          ...
          $('<style> .' + o.chartId + ' .google-visualization-tooltip span { ' + 'color: ' + o.cchartOptions.tooltip.textStyle.color + ' !important; ' + 'font-size: ' + o.cchartOptions.tooltip.textStyle.fontSize + 'px !important; ' + 'font-family: "' + o.cchartOptions.tooltip.textStyle.fontName + '" !important; ' + '} </style>').appendTo('head');
        }
      });
    ...
    o.formatSheet = function (data) {
      ...
      var dataArray = []; // Format Google Sheet csv data
      ...
      if (data && !data.statusText) {
        try {
          ...
          columns = rows[0]; // Get and format columns
          ...
          dataArray = rows;
          dataArray[0] = columns; // Format data
    ...
    o.getTableData = function ($tableHTML) {
      ...
      try {
        ...
        var $rows = $tableHTML.find('tr').not($tableHTML.find('tfoot tr')); // The array of data collected from all rows

        var rowsArr = []; // Add cells as objects to rowsArr

        $rows.each(function (row) {
          rowsArr.push([]);
          $(this).find('td, th').each(function (col) {
            var $cell = $(this);
            var cellObj = {}; // Construct the cell object
            ...
            cellObj.label = $cell.text(); // add cell object to rowsArr
        ...
        dataTable.push(rowsArr[0]); // Change cell data back to values - excluding headings
    ...
    o.formatData = function (data) {
      ...
      try {
        ...
        var removeColArr = []; // Dynamic column types array - An array listing the active dynamic column types

        var dynColTypesArr = []; // The dynamic columns Array - An array of objects containing dynamic column metadata

        var dynColumns = []; // Remove ignore rows filter function

        var filterRows = function filterRows(n, i) {
          return o.options.ignoreRow.indexOf(i) === -1;
        }; // Remove ignore columns filter function


        var filterCols = function filterCols(n, i) {
          return o.options.ignoreCol.indexOf(i) === -1;
        }; // Remove dynamic columns filter function


        var filterDynamic = function filterDynamic(n, i) {
        ...
        formattedData = $.grep(formattedData, filterRows); // Remove ignored columns
        ...
        if (dynColTypesArr.length) {
          ...
          var domain = ''; // Get column indexes to remove

          for (var i = 0; i < formattedData[0].length; i++) {
            // each column heading
            var role = formattedData[0][i].role || ''; // Add annotation and tooltip column indexes to remove to removeColArr
          ...
          for (var i = 0; i < formattedData[0].length; i++) {
            ...
            var label = formattedData[0][i].label || ''; // Get domain for use with tooltip and annotation

            if (role === 'domain' || i === 0) {
              domain = formattedData[0][i];
            }

            if ((!role || dynColTypesArr.indexOf(role) === -1) && i > 0) {
              // not a dynamic column
              // Add dynamic column if needed
              for (var j = 0; j < dynColTypesArr.length; j++) {
                // each dynamic col type
                i++; // Insert the dynamic column
          ...
          for (var i = 1; i < formattedData.length; i++) {
            ...
            for (var j = 0; j < dynColumns.length; j++) {
              // each dynamic column
              var dynData = ''; // Get dynamic column type
              ...
              dynData = dynData.replace(new RegExp('data', 'g'), formattedData[i][dynColumns[j].index]); // Insert into formattedData
        ...
        for (var i = 1; i < formattedData.length; i++) {
          ...
          for (var j = 0; j < formattedData[i].length; j++) {
            ...
            var cellData = formattedData[i][j] || ''; // Format data and add to cellData array

            if (['tooltip', 'annotation'].indexOf(colRole) === -1) {
              // Not a tooltip/annotation
    ...
    o.generateTable = function (data, tableTitle, tableId, tableClass) {
      // The html string
      var html = '<table id="' + tableId + '" class="' + tableId + ' ' + tableClass + '"><caption>' + tableTitle + '</caption><thead><tr>'; // Add header columns to html
      ...
      html += '</tr></thead><tbody>'; // Add rows to html
        ...
        'resize': function resize() {
          ...
          o.timer = setTimeout(function () {
            ...
            if ($(window).width() !== o.windowWidth) {
              ...
              var elStyle = $chartS.attr('style'); // Remove js styles from chart

              $chartS.removeAttr('style'); // Test if chart parent has changed width
              ...
              if (o.chartParent.width() !== o.chartParentWidth) {
                ...
                o.setDimensions(); // Redraw chart ---------------------------------------------

                o.chart.draw(o.data, o.cchartOptions);
              } else {
                // parent has not changed width
    ...
    o.showTableChart = function (table, chart) {
      //  Values: 'show', 'hide', or 'remove'
      var tableLen = $tableS ? $tableS.length : false;
      var chartLen = $chartS ? $chartS.length : false; // Table
    ...
    o.setDimensions = function () {
      ...
      o.chartParentWidth = o.chartParent.width(); // Set chart width and height
