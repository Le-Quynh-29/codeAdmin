...
(function ($, window, document, undefined) {
  ...
  var defaults = {
    ChartType: "Line",
    //Area, Scattered, Bar, Hybrid, Pie, Stacked, Stacked Hybrid
    xPadding: 60,
    yPadding: 50,
    topmargin: 25,
    rightmargin: 20,
  ...
  Plugin.prototype = {
    ...
    drawAxis: function drawAxis(c, graph) {
      var that = this,
    ...
    drawPie: function drawPie(c, graph) {
      var that = this,
          config = this.options;
      ...
      for (var i = 0; i < config.data[0].values.length; i++) {
        ...
        c.arc(centerx, centery, config.legendposition == 'right' || config.legendposition == 'left' ? centerx : centery, lastend, lastend + Math.PI * 2 * (config.data[0].values[i].Y / totalVal), false);
    ...
    drawBar: function drawBar(c, graph) {
      var that = this,
          config = this.options;
    ...
    drawStacked: function drawStacked(c, graph) {
      var that = this,
          config = this.options;
    ...
    drawHybrid: function drawHybrid(c, graph) {
      var that = this,
          config = this.options;
      var randomcolor;
    ...
    drawLineAreaScatteredHybridCharts: function drawLineAreaScatteredHybridCharts(c, graph) {
      var that = this,
          config = this.options;
    ...
    drawLegends: function drawLegends(graph) {
      var that = this,
          config = this.options;
      ...
      if (config.ChartType == "Line" || config.ChartType == "Area" || config.ChartType == "Scattered" || config.ChartType == "Stacked" || config.ChartType == "StackedHybrid") {
        var _legends = $("<div class='simple-chart-legends' />", {
          id: "legendsdiv"
        }).css({
